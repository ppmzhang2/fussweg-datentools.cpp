...
std::vector<ibox::ImgBox> ibox::fromViaCsv(std::istream &stream_i) {
...
// }
// ```
std::vector<ibox::ImgBox> ibox::fromViaJson(std::istream &stream_i) {
...
std::vector<ibox::ImgBox> ibox::fromVia(const std::string &dir) {
    ...
    for (const auto &f : fdt::utils::listAllFiles(dir, ".csv")) {
        ...
        if (stream.is_open()) {
            std::vector<ibox::ImgBox> iboxes_csv = ibox::fromViaCsv(stream);
    ...
    for (const auto &f : fdt::utils::listAllFiles(dir, ".json")) {
        std::ifstream stream(f);
        if (stream.is_open()) {
            std::vector<ibox::ImgBox> iboxes_json = ibox::fromViaJson(stream);
