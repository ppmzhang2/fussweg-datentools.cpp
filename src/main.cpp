...
int parse_args(int argc, char *argv[]) {
    if (argc <= 1) {
        ...
        std::cout << "  " << argv[0] << " crop-bbox "
                  ...
                  << std::endl;
        std::cout << "  " << argv[0] << " draw-via-bbox "
                  << "<label_dir> <src_dir> <dst_dir>" << std::endl;
    ...
    if (op != "exif-export-json" && op != "exif-export-csv" &&
        ...
        op != "fault-stats-via" && op != "annot-to-coco" &&
        op != "crop-bbox" && op != "draw-via-bbox" && op != "pov-roi" &&
