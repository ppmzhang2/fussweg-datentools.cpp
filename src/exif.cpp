...
/*
 * Convert to a double coordinate (i.e. latitude or longitude) from a string of
 * the form:
 *
 ...
 */
inline OptDbl get_coor(const OptStr &str, const OptStr &ref) {
    if (!str) {
        return std::nullopt;
    }
...
inline OptInt to_int(const OptStr &str) {
    if (!str || str->empty()) {
        return std::nullopt;
    }
    return std::stoi(*str);
...
inline OptStr get_model(const OptStr &make, const OptStr &model) {
    if (!make || !model) {
        return std::nullopt;
    }
    return *make + " " + *model;
 ...
 */
inline OptDbl get_coc_from_model(const OptStr &model) {
...
/*
 * Compute the hyperfocal distance (in meters) from the focal length (in mm),
 * aperture (f-number), and circle of confusion (in mm).
 * formula: H = f^2 / (n * c) + f
 ...
 */
inline double get_hyperfocal_dist(OptDbl f, OptDbl n, OptDbl c) {
    if (!f || !n || !c) {
        return 0;
    }
...
/*
 * Convert a date and time string to a std::tm object; return std::nullopt if
 * either the date or time string is null.
 ...
 */
inline OptTm get_gps_ts(const OptStr &ymd, const OptStr &hms) {
    if (!ymd || !hms) {
        return std::nullopt;
    }
    std::tm ts = {};
 ...
 * @throws std::runtime_error if the image cannot be opened or if no EXIF data
 * is found
 */
...
/*
 * Extract the EXIF attributes from an image
 *
 * @param path: the path to the image
 * @return: the ExifAttrs object
 * @throws std::runtime_error if the image cannot be opened or if no EXIF data
 * is found
 */
ExifAttrs GetExifAttrs(const std::string &path) {
    ...
    attrs.exif_ver = get_attr_str(dat, ExifKeys::EXIF_VER);
    attrs.model = get_model(get_attr_str(dat, ExifKeys::MAKE),
                            get_attr_str(dat, ExifKeys::MODEL));
    attrs.desc = get_attr_str(dat, ExifKeys::DESC);
    attrs.height = to_int(get_attr_str(dat, ExifKeys::HEIGHT));
    attrs.width = to_int(get_attr_str(dat, ExifKeys::WIDTH));
    attrs.lat = get_coor(get_attr_str(dat, ExifKeys::LAT),
                         get_attr_str(dat, ExifKeys::LAT_REF));
    attrs.lon = get_coor(get_attr_str(dat, ExifKeys::LON),
                         get_attr_str(dat, ExifKeys::LON_REF));

    attrs.altitude = frac(get_attr_str(dat, ExifKeys::ALTITUDE));
    attrs.ts_gps = get_gps_ts(get_attr_str(dat, ExifKeys::GPS_DATE),
                              get_attr_str(dat, ExifKeys::GPS_TIME));
    attrs.coc = get_coc_from_model(attrs.model);
    attrs.exposure_time = frac(get_attr_str(dat, ExifKeys::EXPOSURE_TIME));
    attrs.iso = to_int(get_attr_str(dat, ExifKeys::ISO));
    attrs.shutter_speed = frac(get_attr_str(dat, ExifKeys::SHUTTER_SPEED));
    attrs.aperture = frac(get_attr_str(dat, ExifKeys::APERTURE));
    attrs.subj_dist = frac(get_attr_str(dat, ExifKeys::SUBJ_DIST));
    attrs.focal_length = frac(get_attr_str(dat, ExifKeys::FOCAL_LENGTH));
